...
public class TemplateHandler extends WebHandler {
	...
	protected void renderTemplate(WebRequest req, WebResponse res,
			String tempPath) throws IOException {
		renderTemplate(req, res, tempPath, new TypeMap());
	}

	protected void renderTemplate(WebRequest req, WebResponse res,
			String tempPath, TypeMap data) throws IOException {
		File tempFile = new File(WebServer.webRoot, tempPath.replace('/',
				File.separatorChar));
		renderTemplate(req, res, tempFile, data);
	}

	protected void renderTemplate(WebRequest req, WebResponse res, File tempFile)
			throws IOException {
		renderTemplate(req, res, tempFile, new TypeMap());
	}

	protected void renderTemplate(WebRequest req, WebResponse res,
			File tempFile, TypeMap data) throws IOException {
		if (!tempFile.getParentFile().getAbsolutePath()
				.startsWith(WebServer.webRoot.getAbsolutePath())) {
			res.setStatus(403, "Forbidden");
			return;
		}
		String relPath = tempFile.getAbsolutePath().substring(
				WebServer.webRoot.getAbsolutePath().length());
		if (!tempFile.isFile()) {
		...
		try {
			...
			Utils.severe(t, "Exception while loading web server template %s:",
		...
		try {
			...
			res.setStatus(500, "Server error");
			Utils.severe(e, "Exception while processing web server view %s:",
					tempFile.getAbsolutePath());
			return;
		}
