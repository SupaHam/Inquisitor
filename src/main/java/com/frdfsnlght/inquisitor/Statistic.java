...
public final class Statistic {
	...
	public Statistic(String name, Type type, boolean mapped, int size,
			String def) {
		if (!name.matches("^\\w+$"))
			throw new IllegalArgumentException("'" + name
					+ "' is not a valid statistic name");
		if (mapped) {
			size = -1;
			def = null;
		} else
	...
	public String toString() {
		return "Statistic[" + name + "," + type.toString()
				+ (mapped ? "(M)" : "") + "]";
	}
	...
	public void validate() {
		...
		if (mapped)
			group.validateColumn(MappedObjectsColumn, Type.OBJECT, -1, null,
					null);
		else
	...
	public enum Type {
		STRING("varchar"), BOOLEAN("tinyint"), OBJECT("longtext"), TIMESTAMP(
				"timestamp"), ELAPSED_TIME("float"), DISTANCE("float"), INTEGER(
				"int"), LONG("bigint"), FLOAT("float"), DOUBLE("double");
		...
		public String getSQLDef() {
			...
			case STRING:
				throw new UnsupportedOperationException(
						"cannot get definition of STRING without a size");
			case BOOLEAN:
		...
		/*
		 * public String getSQLType() { return String.format(columnDefFormat,
		 * defaultSize); }
		 * 
		 * public String getColumnDef(int size) { if (size == -1) size =
		 * defaultSize; return String.format(columnDefFormat, size); }
		 * 
