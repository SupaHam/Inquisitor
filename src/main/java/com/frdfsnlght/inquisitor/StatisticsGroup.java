...
public final class StatisticsGroup {
	...
	public StatisticsGroup(String name, String keyName, Type keyType,
			int keySize) {
		if (!name.matches("^\\w+$"))
			throw new IllegalArgumentException("'" + name
					+ "' is not a valid statistics group name");
		if (!keyName.matches("^\\w+$"))
			throw new IllegalArgumentException("'" + keyName
					+ "' is not a valid statistics group key name");
		switch (keyType) {
		case STRING:
			break;
		case INTEGER:
		case LONG:
			...
			break;
		default:
			throw new IllegalArgumentException("type '" + keyType
	...
	public void setDeleteInterval(long interval) {
		if (interval < 1)
			throw new IllegalArgumentException(
					"delete interval must be greater than 0");
	...
	public TypeMap loadStatistics(ResultSet rs, Collection<String> statNames)
			throws SQLException {
		...
		for (String statName : statNames) {
			...
			if (stat.isMapped()) {
				...
				case DOUBLE:
					...
					break;
				default:
					throw new UnsupportedOperationException(stat
	...
	public void flush() {
		for (Statistics stat : stats.values())
			if ((stat.getLastFlushed() + flushInterval) < System
					.currentTimeMillis())
				stat.flush();
	}
	...
	public void validate() {
		...
		try {
			...
			Utils.severe("SQLException while validating %s: %s", this,
					se.getMessage());
		} finally {
			try {
				if (rs != null)
	...
	public boolean validateColumn(String colName, Type type, int size,
			String def, Set<String> oldNames) {
		if (!DB.isConnected())
			return false;
		...
		try {
			...
			Utils.severe("SQLException while validating column %s in %s: %s",
					colName, this, se.getMessage());
			return false;
		} finally {
			try {
				if (rs != null)
