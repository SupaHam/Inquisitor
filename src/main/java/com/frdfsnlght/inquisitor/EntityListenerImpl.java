...
public final class EntityListenerImpl implements Listener {
	...
	public void onEntityDamage(EntityDamageEvent event) {
		...
		Statistics stats = PlayerStats.group.getStatistics(((Player) event
				.getEntity()).getName());
		stats.add("travelDistances", "Falling", event.getEntity()
				.getFallDistance());
	}
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		EntityDamageByEntityEvent killEvent = (EntityDamageByEntityEvent) deadEnt
				.getLastDamageCause();
		...
		String weaponName = ((weapon == null) || (weapon == Material.AIR)) ? "None"
				: Utils.titleCase(weapon.toString());
		...
		if (deadEnt instanceof Player) {
			...
			String deadName = getEntityName(deadEnt);
	...
	public void onEntityShootBow(EntityShootBowEvent event) {
		if (!(event.getEntity() instanceof Player))
			return;
		PlayerStats.group.getStatistics(((Player) event.getEntity()).getName())
				.incr("arrowsShot");
	}
	...
	public void onEntityTame(EntityTameEvent event) {
		if (!(event.getOwner() instanceof Player))
			return;
		PlayerStats.group.getStatistics(((Player) event.getOwner()).getName())
				.incr("animalsTamed",
						Utils.normalizeEntityTypeName(event.getEntityType()));
	}
	...
	public void onFoodLevelChange(FoodLevelChangeEvent event) {
		...
		if (!food.isEdible())
			return;
		PlayerStats.group.getStatistics(player.getName()).incr("foodEaten",
				Utils.titleCase(food.name()));
	}
