...
public final class SkinHandler extends TemplateHandler {
	...
	public void handleRequest(WebRequest req, WebResponse res)
		...
		if (cacheFile == null) {
			...
			Utils.warning("Unable to download default skin from %s%s.png!",
					SKIN_URL, DEFAULT_SKIN);
			return;
		}
	...
	private File cacheSkin(String name) throws IOException {
		...
		File file = new File(cacheDir, URLEncoder.encode(name, "US-ASCII")
				+ ".png");
		if (file.exists()
				&& ((System.currentTimeMillis() - file.lastModified()) <= CACHE_REFRESH))
			return file;
		URL url = new URL(SKIN_URL + URLEncoder.encode(name, "US-ASCII")
				+ ".png");
		if (Utils.downloadFile(url, file, false))
			return file;
		return null;
	}
