...
public class TestCommands extends CommandProcessor {
	...
	@Override
	public void process(final Context ctx, Command cmd, List<String> args)
			throws InquisitorException {
		args.remove(0);
		if (args.isEmpty())
		...
		if ("enchant".startsWith(subCmd)) {
			...
			try {
				...
				throw new CommandException("%s value for enchantment type",
						e.getMessage());
			}
			int level = 1;
			if (!args.isEmpty()) {
				...
				try {
					...
					throw new CommandException("%s is not a valid level",
							levelStr);
				}
			}
			...
			if (!ench.canEnchantItem(is))
				throw new CommandException("%s cannot be enchanted with %s",
						is.getType(), enchs);
			if (ench.getMaxLevel() < level)
				throw new CommandException("%s cannot be used at that level",
						enchs);
		...
		if ("stats".startsWith(subCmd)) {
			...
			if (statsGroup == null) {
				statsGroup = new StatisticsGroup("stats", "name", Type.STRING,
						30);
				statsGroup.addStatistic(new Statistic("blahs", Type.INTEGER,
						false));
			}
	...
	private enum Enchantments {
		...
		PROTECTION_ENVIRONMENTAL(Enchantment.PROTECTION_ENVIRONMENTAL), PROTECTION_FIRE(
				Enchantment.PROTECTION_FIRE), PROTECTION_FALL(
				Enchantment.PROTECTION_FALL), PROTECTION_EXPLOSIONS(
				Enchantment.PROTECTION_EXPLOSIONS), PROTECTION_PROJECTILE(
				Enchantment.PROTECTION_PROJECTILE), OXYGEN(Enchantment.OXYGEN), WATER_WORKER(
				Enchantment.WATER_WORKER), DAMAGE_ALL(Enchantment.DAMAGE_ALL), DAMAGE_UNDEAD(
				Enchantment.DAMAGE_UNDEAD), DAMAGE_ARTHROPODS(
				Enchantment.DAMAGE_ARTHROPODS), KNOCKBACK(Enchantment.KNOCKBACK), FIRE_ASPECT(
				Enchantment.FIRE_ASPECT), LOOT_BONUS_MOBS(
				Enchantment.LOOT_BONUS_MOBS), DIG_SPEED(Enchantment.DIG_SPEED), SILK_TOUCH(
				Enchantment.SILK_TOUCH), DURABILITY(Enchantment.DURABILITY), LOOT_BONUS_BLOCKS(
				Enchantment.LOOT_BONUS_BLOCKS), ARROW_DAMAGE(
				Enchantment.ARROW_DAMAGE), ARROW_KNOCKBACK(
				Enchantment.ARROW_KNOCKBACK), ARROW_FIRE(Enchantment.ARROW_FIRE), ARROW_INFINITE(
