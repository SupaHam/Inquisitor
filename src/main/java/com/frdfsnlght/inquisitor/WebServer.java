...
public final class WebServer {
	...
	static {
		...
		options = new Options(WebServer.class, OPTIONS, "inq.webserver",
				new OptionsListener() {
					@Override
					public void onOptionSet(Context ctx, String name,
							String value) {
						ctx.sendLog("web server option '%s' set to '%s'", name,
								value);
	...
	public static void start() {
		...
		try {
			...
			if (!webRoot.isDirectory()) {
				if (Utils.installManifest("/resources/web_root/manifest",
						webRoot, false))
			...
			if (ROUTES.isEmpty()) {
				...
				ROUTES.add(new WebRoute(new PlayerHandler(), "^/player/(.*)",
						"playerName"));
				ROUTES.add(new WebRoute(new PlayerSearchHandler(),
						"^/playerSearch/(.*)", "playerName"));
				ROUTES.add(new WebRoute(new PlayersHandler(), "^/players/?$"));
				ROUTES.add(new WebRoute(new SkinHandler(), "^/skin/(.*)",
						"playerName"));
				ROUTES.add(new WebRoute(new RedirectHandler("/", 303),
						"^/resources/"));
						...
						synchronized (workerThreads) {
							...
							if (started) {
								if (System.currentTimeMillis() >= expireTime) {
									...
									Utils.debug(
											"web server accepted connection from %s",
		...
		serverThread.start();
		Utils.info("web server started on %s:%s", (getAddress() == null) ? "*"
				: getAddress(), getPort());
	}
	...
	public static String getPlayersColumns() {
		return Config.getStringDirect("webServer.playersColumns");
	}
	...
	public static void setPlayersColumns(String s) {
		...
		Config.setPropertyDirect("webServer.playersColumns", s);
	}

	public static String getPlayersSortColumn() {
		return Config.getStringDirect("webServer.playersSortColumn");
	}
	...
	public static String getPlayersSortDir() {
		return Config.getStringDirect("webServer.playersSortDir");
	}
	...
	public static String getPlayersRestrictColumns() {
		return Config.getStringDirect("webServer.playersRestrictColumns");
	}
	...
	public static void setPlayersRestrictColumns(String s) {
		...
		Config.setPropertyDirect("webServer.playersRestrictColumns", s);
	}
