...
public final class PlayerHandler extends TemplateHandler {
	...
	private TypeMap getPlayer(String name) {
		...
		try {
			...
			sql.append("SELECT `").append(Statistic.MappedObjectsColumn)
					.append('`');
			for (String statName : statNames) {
				if (PlayerStats.group.getStatistic(statName).isMapped())
					continue;
			...
			Utils.severe("SQLException while selecting player '%s': %s", name,
					se.getMessage());
			return null;
		} finally {
			try {
