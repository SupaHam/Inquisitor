...
public final class JSON {
	...
	private static String encodeValue(Object o) {
		...
		if (o instanceof Map)
			return encodeObject((Map) o);
		if (o instanceof Collection)
			return encodeArray((Collection) o);
		if (o.getClass().isArray())
			return encodeArray((Object[]) o);

		throw new IllegalArgumentException("unsupported JSON encodable object "
				+ o);
	}
	...
	private static Object decodeValue(StringBuilder sb, int[] pos) {
		...
		switch (ch) {
		case '{':
			return decodeObject(sb, pos);
		case '[':
			return decodeArray(sb, pos);
		case '"':
			return decodeString(sb, pos, ch);
		case '\'':
			return decodeString(sb, pos, ch);
		default:
			...
			throw new IllegalArgumentException("unexpected JSON syntax at "
					+ pos[0] + " (" + sb.substring(0, 200) + ")");
		}
	}
		...
		for (;;) {
			...
			if (ch == ':')
				...
				throw new IllegalArgumentException(
						"expected object key/value separator in JSON at "
								+ pos[0]);
			...
			if (ch == '\\') {
				...
				case 'u':
					...
					if (!hex.matches("^[a-f0-9]{4}$"))
						throw new IllegalArgumentException(
								"expected hexidecimal digits in JSON at "
										+ pos[0]);
					str.append(Character.toChars(Integer.parseInt(hex, 16)));
					break;
				default:
					// any '\' preceded character will be replaced with the
					// character
					str.append(ch);
					break;
				}
	...
	private static Number decodeNumber(StringBuilder sb, int[] pos) {
		...
		throw new IllegalArgumentException("expected number in JSON at "
				+ pos[0]);
	}
	...
	private static void requireChars(StringBuilder sb, int[] pos) {
		if (sb.length() == 0)
			throw new IllegalArgumentException("unexpected end of JSON at "
					+ pos[0]);
	}
	...
	private static String stripChars(StringBuilder sb, int[] pos, int count) {
		if (count > sb.length())
			throw new IllegalArgumentException("expected " + count
					+ " characters at end of JSON at " + pos[0]);
