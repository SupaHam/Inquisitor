...
public final class PlayerStats {
	...
	public static void onPlayerJoin(Player player) {
		...
		if ((bedServer != null)
				&& bedServer.equals(Global.plugin.getServer().getServerName()))
	...
	public static void onPlayerMove(Player player, Location to) {
		...
		if (state.lastTime != 0)
			elapsed = (System.currentTimeMillis() - state.lastTime) / 1000.0f;
	...
	private static void savePlayerInfo(Statistics stats) {
		...
		PlayerState state = playerStates.get(player.getName());
		if (state != null) {
			float sessionTime = (System.currentTimeMillis() - state.joinTime) / 1000f;
	...
	private static Object encodeItemStacks(ItemStack[] stacks) {
		...
		for (int slot = 0; slot < stacks.length; slot++) {
			...
			if (stack == null)
				...
				net.minecraft.server.ItemStack mcStack = ((CraftItemStack) stack)
						.getHandle();
				if (mcStack != null)
					stackMap.put("tag", NBT.encodeNBT(mcStack.getTag()));
	...
	private static enum TravelMode {
		WALKING, SPRINTING, SNEAKING, FLYING, SWIMMING, RIDING, RIDING_MINECART, RIDING_PIG, RIDING_BOAT;
	}
