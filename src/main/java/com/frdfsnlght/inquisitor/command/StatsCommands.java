...
public class StatsCommands extends CommandProcessor {
	...
	@Override
	public void process(Context ctx, Command cmd, List<String> args)
		...
		if ("status".startsWith(subCmd)) {
			...
			ctx.send("statistics manager %s started",
					StatisticsManager.isStarted() ? "is" : "is not");
			...
			for (StatisticsGroup group : groups) {
				...
				ctx.send("  group '%s' has %s statistics, %s instances cached",
						group.getName(), group.getStatistics().size(),
						stats.size());
				for (Statistics s : stats)
					ctx.send("    '%s' has %s dirty statistics %s", s.getKey(),
							s.getDirty().size(), s.isValid() ? "" : "(invalid)");
			}
		...
		if ("flush".startsWith(subCmd)) {
			...
			if (!StatisticsManager.isStarted())
				throw new CommandException(
						"statistics manager has not been started");
			if (args.isEmpty()) {
			...
			if (stats == null)
				throw new CommandException("unknown key '%s' in group '%s'",
						key, groupName);
			stats.flush();
			return;
		}
