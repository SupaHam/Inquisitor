...
public final class PlayersHandler extends TemplateHandler {
	...
	private static final String DEFAULT_COLUMNS = "level," + "deaths,"
			+ "totalPlayersKilled," + "totalMobsKilled," + "totalBlocksBroken,"
			+ "totalBlocksPlaced," + "totalItemsDropped,"
			+ "totalItemsPickedUp," + "totalItemsCrafted,"
			+ "totalDistanceTraveled," + "totalTime," + "online";

	private static final String DEFAULT_SORT_COLUMN = "name";
	private static final String DEFAULT_SORT_DIR = "ASC";

	private static final String DEFAULT_RESTRICT_COLUMNS = "address,"
			+ "coords," + "bedServer," + "bedWorld," + "bedCoords";
	...
	public void handleRequest(WebRequest req, WebResponse res)
		...
		if (defColumns == null)
			defColumns = DEFAULT_COLUMNS;
		String defSortColumn = WebServer.getPlayersSortColumn();
		if (defSortColumn == null)
			defSortColumn = DEFAULT_SORT_COLUMN;
		String defSortDir = WebServer.getPlayersSortDir();
		if (defSortDir == null)
			defSortDir = DEFAULT_SORT_DIR;
		String defRestrictColumns = WebServer.getPlayersRestrictColumns();
		if (defRestrictColumns == null)
			defRestrictColumns = DEFAULT_RESTRICT_COLUMNS;

		String columnsStr = req.getParameter("columns", defColumns);
		String sortColumn = req.getParameter("sortBy", defSortColumn);
		String sortDir = req.getParameter("sortDir", defSortDir).toUpperCase();
		int pageSize = req.getParameters().getInt("pageSize",
				WebServer.getPlayersPageSize());
		int pageNum = req.getParameters().getInt("page", 1);
		String playerName = req.getParameter("playerName", null);

		Set<String> restrictColumns = new HashSet<String>(
				Arrays.asList(defRestrictColumns.split("\\s*,\\s*")));
		...
		for (String statName : PlayerStats.group.getStatisticsNames()) {
			...
			case FLOAT:
			case DOUBLE:
				if (!restrictColumns.contains(statName))
					allColumns.add(statName);
				break;
			}
		}
		...
		try {
			...
			sql.append("SELECT COUNT(1) FROM ").append(
					DB.tableName(PlayerStats.group.getName()));
			if (playerName != null)
			...
			int totalPages = (int) Math.ceil((double) totalPlayers
					/ (double) pageSize);
			if (totalPages < 1)
				totalPages = 1;
			...
			if (!"name".equals(sortColumn))
				sql.append(", `name` ASC");
			sql.append(" LIMIT ").append(limitOffset).append(",")
					.append(pageSize);
			stmt = DB.prepare(sql.toString());
			if (playerName != null)
				stmt.setString(1, "%" + playerName + "%");
			...
			Utils.severe("SQLException while selecting players: %s",
					se.getMessage());
		} finally {
			try {
				if (rs != null)
